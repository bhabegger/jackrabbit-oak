...
/**
 * Lucene 9.x implementation of IndexReader.
 * Wraps Lucene's DirectoryReader for version-agnostic access.
 */
public final class Lucene9IndexReader implements IndexReader {
     ...
     */
    public Lucene9IndexReader(Directory directory) throws IOException {
        if (directory == null) {
     ...
     */
    public Lucene9IndexReader(DirectoryReader reader) {
        if (reader == null) {
    ...
    public Document document(int docID) throws IOException {
        ...
        org.apache.lucene.document.Document luceneDoc = delegate.storedFields().document(docID);
        return new Lucene9Document(luceneDoc);
    }
     ...
     */
    DirectoryReader getLuceneReader() {
        return delegate;
    }
}
