...
/**
 * Oak {@link Result} API tests for faceting (Lucene 9). JCR-level facet parity with legacy Lucene / Elastic is covered
 * by {@link LuceneNgFacetCommonTest} ({@link org.apache.jackrabbit.oak.plugins.index.FacetCommonTest}).
 * This harness uses {@link Result} rows, which do not carry {@code rep:facet(...)} values the same way as
 * {@link javax.jcr.query.QueryResult}, so assertions stay disabled until that gap is closed.
 */
@Ignore("Oak Result rows omit rep:facet JSON; see LuceneNgFacetCommonTest for JCR facet coverage")
public class LuceneNgFacetTest extends AbstractQueryTest {
    ...
    private FacetResult executeFacetQuery(String query) throws ParseException {
        ...
        FacetResult.FacetResultRow[] facetRows = new FacetResult.FacetResultRow[rows.size()];
        for (int i = 0; i < rows.size(); i++) {
            ResultRow currentRow = rows.get(i);
            facetRows[i] = columnName -> {
                PropertyValue pv = currentRow.getValue(columnName);
                return pv == null ? null : pv.getValue(Type.STRING);
            };
        }
        return new FacetResult(columnNames, facetRows);
    }
