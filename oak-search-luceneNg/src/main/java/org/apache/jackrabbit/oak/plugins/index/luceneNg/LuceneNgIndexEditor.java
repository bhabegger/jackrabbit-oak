...
public class LuceneNgIndexEditor implements Editor {
    ...
    public Editor childNodeAdded(@NotNull String name, @NotNull NodeState after)
        ...
        return new LuceneNgIndexEditor(childPath, indexPath, definition, root,
                indexWriter, indexDefinition, callback);
    }
    ...
    private void indexNode(NodeState node) throws IOException {
        ...
        // FacetsConfig.build() processes SortedSetDocValuesFacetField entries.
        Map<String, Integer> facetDimCounts = new HashMap<>();
        for (org.apache.lucene.index.IndexableField field : doc.getFields()) {
            if (field instanceof SortedSetDocValuesFacetField) {
                ...
                facetDimCounts.merge(dim, 1, Integer::sum);
            }
        }
        FacetsConfig facetsConfig = new FacetsConfig();
        for (Map.Entry<String, Integer> e : facetDimCounts.entrySet()) {
            String dim = e.getKey();
            facetsConfig.setIndexFieldName(dim, FieldNames.createFacetFieldName(dim));
            if (e.getValue() > 1) {
